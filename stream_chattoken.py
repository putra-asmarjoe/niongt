...
def show_result_query(user_query):
        ...
        with st.container():  # Gunakan container untuk menempatkan teks di tengah 
            st.info("Faiss Similiar Text: %s" % (most_similar_text2), icon="ℹ️")  # Tampilkan teks    
            st.info("Faiss tokens: %s \n\nGpt tokens: %s" % (numtoken, ttlgpttoken), icon="ℹ️")  # Tampilkan teks   
      ...
      with st.sidebar:
          ...
          if st.button("Process"):
            ...
            with st.spinner("Processing"):
                ...
                file_path = os.path.join(folder_path, pdf_docs[0].name)
                with open(file_path, "wb") as f:
                    f.write(pdf_docs[0].getbuffer())
      ...
      if sapa_key == "":
        ...
        if(sapa_token != sapa_key):
            ...
            if "docname" not in st.session_state:        
              ...
              st.session_state["messages"] = [{"role": "assistant", "content": "Anda telah mengupload file %s , Mulai chat untuk berbicara dengan data anda"% (st.session_state["docname"])}]
